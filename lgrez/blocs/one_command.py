...
#: Commands exempted from one_command limitation,
#: registered by :func:`.do_not_limit`
exempted: list[commands.Command] = []


class AlreadyInCommand:
...
async def not_in_command(ctx: commands.Context) -> bool:
    ...
    Args:
        ctx: contexte d'invocation de la commande.
...
# @bot.before_invoke
async def add_to_in_command(ctx: commands.Context) -> None:
    """Ajoute le channel à la liste des channels dans une commande.

    Fonction à appeler avant chaque appel de fonction
    (enregistrer avec :meth:`~discord.ext.commands.Bot.before_invoke`)

    Elle est appelée seulement si les checks sont OK, donc pas si le
...
# @bot.after_invoke
async def remove_from_in_command(ctx: commands.Context) -> None:
    """Retire le channel de la liste des channels dans une commande.

    Fonction à appeler après chaque appel de fonction.
    ...
    if ctx.channel.id in config.bot.in_command and ctx.command not in exempted:
...
def bypass(ctx: commands.Context) -> _Bypasser:
    """Context manager: bypass one-command limitation.

    Args:
        ctx: running command context.
...
def do_not_limit(command: commands.Command) -> commands.Command:
