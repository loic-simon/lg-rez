...
from lgrez.blocs.bdd import base
...
_baseaction_role = sqlalchemy.Table('_baseactions_roles', base.TableBase.metadata,
...
class Camp(base.TableBase):
    ...
    #: :class:`str`: nom de l'emoji associé au camp (devrait être un emoji valide du serveur) (``len <= 32``)
    ...
    def discord_emoji(self):
        """:class:`discord.Emoji`: Emoji Discord correspondant à ce camp

        Raises:
            :exc:`ValueError`: :attr:`.emoji` non défini ou manquant sur le serveur
            :exc:`RuntimeError`: bot non connecté (:attr:`.config.guild` vaut ``None``)
        """
        if not self.emoji:
        ...
        try:
            ...
            raise ValueError(f"Pas d'emoji :{self.emoji}: sur le serveur !")

    @property
    def discord_emoji_or_none(self):
        """:class:`discord.Emoji` ou ``None``: Emoji Discord correspondant à ce camp, si défini
